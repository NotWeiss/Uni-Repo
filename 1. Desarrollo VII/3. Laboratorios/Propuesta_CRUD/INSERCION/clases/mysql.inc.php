...
class mod_db{
	...
	var $total;
	var $pagecut_query;
	var $conn_type = "1"; // 0: mysqli_connect ; 1: mysqli_pconnect
	...
	function connection(){
		...
		if(!$this->sql_link){
			...
			// SQL Connection
				if($this->conn_type == 0){
					$this->sql_link = mysqli_connect($this->db_host,$this->db_user,$this->db_pass);
			...
			// Select Database
			$this->db_link = mysqli_select_db($this->sql_link, $this->db_name);
	...
	function insert($tb_name,$cols,$val,$astriction){
		...
		$insert = mysqli_query($this->sql_link, "INSERT INTO $tb_name $cols VALUES($val) $ast");
	...
	function update($tb_name,$string,$astriction){
		...
		$update = mysqli_query($this->sql_link, "UPDATE $tb_name SET $string $ast");
	...
	function del($tb_name,$astriction){
		...
		$del = mysqli_query($this->sql_link, "DELETE FROM $tb_name $ast");
	...
	function query($string){
		...
		$this->query_id = mysqli_query($this->sql_link, $string);
	...
	function nums($string="",$qid=""){
		if($string != ""){
			...
			$this->total = mysqli_num_rows($this->sql_link, $qid);
	...
	function objects($string="",$qid=""){
		if($string != ""){
			$this->query($string);
			$objects = mysqli_fetch_object($this->query_id);
				if($this->debug == 1){
					echo "qid = ".$qid."<br>";
					echo "obj = ".$objects."<br><br>";
				}
		}elseif($qid != ""){
			$objects = mysqli_fetch_object($this->sql_link, $qid);
				if($this->debug == 1){
					echo "qid = ".$qid."<br>";
					echo "obj = ".$objects."<br><br>";
				}
	...
	function insert_id($qid=""){
		if($qid){
			$insert_id = mysqli_insert_id($this->sql_link, $qid);
		}elseif(!$qid){
